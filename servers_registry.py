...
# (ok=True), але спільний файл (з погляду ІНШОЇ машини) лишається
# порожнім - підозра на розбіжність у тому, ЯКИЙ САМЕ шлях кожна машина
# насправді вважає "спільною текою" (дві реальні теки OneDrive під одним
# акаунтом - той самий клас проблеми, що вже й "Открыть папку"/standard_
# menu_cloud цього проєкту). Видно через /control/status без потреби
