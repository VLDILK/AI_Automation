...
def _post_operation_tree_action(payload, timeout=10):
    body = dict(payload)
    body["token"] = paths.REMOTE_CONTROL_TOKEN
    request = urllib.request.Request(
        f"{_BASE_URL}/control/operation_tree_action",
        data=json.dumps(body).encode("utf-8"),
        headers={"Content-Type": "application/json", "User-Agent": _USER_AGENT},
        method="POST",
    )
...
def add_remote_operation_category(parent_action_code, kind, label, product, condition=None, timeout=10):
    return _post_operation_tree_action(
        {
            "op": "add_category", "parent_action_code": parent_action_code, "kind": kind,
            "label": label, "product": product, "condition": condition,
        },
        timeout=timeout,
    )


def rename_remote_operation_category(operation_id, new_label, timeout=10):
    return _post_operation_tree_action(
        {"op": "rename_category", "operation_id": operation_id, "label": new_label}, timeout=timeout
    )


def delete_remote_operation_category(operation_id, timeout=10):
    return _post_operation_tree_action({"op": "delete_category", "operation_id": operation_id}, timeout=timeout)


def add_remote_operation_field(operation_id, field_key, label, timeout=10):
    return _post_operation_tree_action(
        {"op": "add_field", "operation_id": operation_id, "field_key": field_key, "label": label},
        timeout=timeout,
    )


def update_remote_operation_field(field_id, label, timeout=10):
    return _post_operation_tree_action({"op": "update_field", "field_id": field_id, "label": label}, timeout=timeout)


def delete_remote_operation_field(field_id, timeout=10):
    return _post_operation_tree_action({"op": "delete_field", "field_id": field_id}, timeout=timeout)


def add_remote_operation_field_column(
    operation_field_id, sheet, column_key, marker, write_mode="generic", exclude_column_id=None, timeout=10,
):
    return _post_operation_tree_action(
        {
            "op": "add_column", "operation_field_id": operation_field_id, "sheet": sheet,
            "column_key": column_key, "marker": marker, "write_mode": write_mode,
...
def delete_remote_operation_field_column(column_id, timeout=10):
    return _post_operation_tree_action({"op": "delete_column", "column_id": column_id}, timeout=timeout)
